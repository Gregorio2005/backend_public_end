...
- **Base de Datos:** PostgreSQL (ya definida arriba).
- **Frontend:** React.js.
- **Autenticación:** Basada en la tabla `users` y `roles` (usando JWT).

## Reglas de Integridad
1. Cada vez que se registre un insumo en `bill_inputs`, debe estar asociado a un `master_inputs_id` válido.
2. Las inspecciones (tablas `inspection_...`) deben actualizar el estado en el sistema de gestión.
3. El campo `update_at` se gestiona por triggers, no es necesario enviarlo desde el backend.

## Estado Final del Proyecto y Funcionalidades
- [x] **Arquitectura en Capas**: Implementación de Route -> Controller -> Service -> Model.
- [x] **Validación Robusta**: Migración total a **Zod** para validación de esquemas.
- [x] **Gestión de Acceso (Auth)**: Sistema de Login, Perfil `/me` y recuperación de contraseña funcional.
- [x] **Seguridad**: Encriptación de contraseñas con `bcryptjs` y protección de rutas con JWT.
- [x] **Módulos Técnicos**: Gestión completa de las 29 tablas del sistema (Insumos, Inspecciones, Facturación, etc.).
- [x] **Lógica de Integridad**: Procesos automatizados mediante Triggers en la base de datos y validaciones en la capa de Servicios.

## Arquitectura de Carpetas
```text
src/
├── config/          # db.js, envs.js
├── middleware/      # auth.js, errorHandler.js, validator.js (Zod)
├── modules/         # Lógica por tabla (Estructura modular)
...
│   │   ├── users_schema.js
│   │   └── users_routes.js
│   └── ...          # Resto de los módulos (Insumos, Inspecciones, etc.)
...
*   **`server.js`**: Es el punto de entrada de la aplicación. Se encarga de verificar la conexión a la base de datos (usando `testConnection`) e iniciar el servidor en el puerto configurado.
*   **`app.js`**: Centraliza la configuración de Express. Aquí se definen los middlewares globales (CORS, Morgan, JSON), se registran todas las rutas de los módulos y se configura el manejador de errores global.
*   **`package.json`**: Gestiona las dependencias del proyecto (Sequelize, Express, JWT, etc.) y define los scripts de ejecución como `npm run dev`.
*   **`.env.example`**: Archivo de plantilla que muestra las variables de entorno necesarias para que el sistema funcione (credenciales de DB, secretos de seguridad).
*   **`src/config/db.js`**: Gestiona la conexión a PostgreSQL mediante Sequelize. Implementa la lógica para alternar entre la base de datos local y la de Neon, y exporta el pool de conexiones.
*   **`src/config/envs.js`**: Se encarga de leer, centralizar y validar las variables del archivo `.env`, asignando valores por defecto cuando es necesario.

### Middlewares (`src/middleware/`)
*   **`auth.js`**: Protege las rutas privadas verificando la validez del token JWT enviado por el cliente.
*   **`errorHandler.js`**: Captura cualquier error ocurrido en los controladores para enviar una respuesta JSON estandarizada y evitar que el servidor se detenga.
*   **`validator.js`**: Facilita la validación de los datos de entrada (body, query, params) utilizando la librería **Zod**.

### Estructura Modular (`src/modules/`)
Para mantener el orden y la escalabilidad en las 29 tablas, cada una cuenta con una arquitectura de capas bien definida:

*   **`*_routes.js`**: Define los puntos de acceso (endpoints) de la API y asocia cada uno con su validación de esquema, lógica de controlador y middlewares de seguridad.
*   **`*_schema.js`**: Contiene la definición de los esquemas de validación mediante **Zod**, asegurando que los datos de entrada cumplan con los tipos y formatos requeridos.
*   **`*_controller.js`**: Actúa como intermediario entre la red y la lógica de negocio. Extrae los parámetros de la petición y delega la ejecución al servicio correspondiente, retornando la respuesta HTTP final.
*   **`*_service.js`**: Contiene la lógica de negocio del módulo. Aquí se aplican reglas, validaciones de integridad y transformaciones de datos antes de llamar al modelo.
*   **`*_model.js`**: Se encarga exclusivamente de la comunicación con la base de datos PostgreSQL, ejecutando las consultas SQL (Queries).
