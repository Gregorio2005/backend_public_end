...
const InspectionCamerasModel = {
    findAll: async () => {
        const query = 'SELECT * FROM public.inspection_cameras ORDER BY id ASC';
        const { rows } = await pool.query(query);
        return rows;
    },

    findById: async (id) => {
        const query = 'SELECT * FROM public.inspection_cameras WHERE id = $1';
    ...
    create: async (data) => {
        ...
        const query = `INSERT INTO public.inspection_cameras 
    ...
    update: async (id, data) => {
        ...
        const query = `UPDATE public.inspection_cameras 
    ...
    delete: async (id) => {
        const query = 'DELETE FROM public.inspection_cameras WHERE id = $1 RETURNING id';
        const { rows } = await pool.query(query, [id]);
        return rows[0];
    }
};
