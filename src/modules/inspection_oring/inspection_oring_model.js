...
const InspectionOringModel = {
    findAll: async () => {
        const query = 'SELECT * FROM public.inspection_oring ORDER BY id ASC';
        const { rows } = await pool.query(query);
        return rows;
    },

    findById: async (id) => {
        const query = 'SELECT * FROM public.inspection_oring WHERE id = $1';
    ...
    create: async (data) => {
        ...
        const query = `INSERT INTO public.inspection_oring 
    ...
    update: async (id, data) => {
        ...
        const query = `UPDATE public.inspection_oring 
    ...
    delete: async (id) => {
        const query = 'DELETE FROM public.inspection_oring WHERE id = $1 RETURNING id';
        const { rows } = await pool.query(query, [id]);
        return rows[0];
    }
};
