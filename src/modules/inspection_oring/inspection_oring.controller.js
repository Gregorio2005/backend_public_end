const InspectionOringService = require('./inspection_oring.service');
const { notifyRole } = require('../../utils/notifications');

const InspectionOringController = {
    getAll: async (req, res, next) => {
        try {
            const data = await InspectionOringService.getAll(req.query.bill_inputs_id);
            res.json({ success: true, data });
        } catch (error) {
            next(error);
        }
    },
    getOne: async (req, res, next) => {
        try {
            const data = await InspectionOringService.getById(req.params.id);
            res.json({ success: true, data });
        } catch (error) {
            next(error);
        }
    },
    create: async (req, res, next) => {
        try {
            const data = await InspectionOringService.create(req.body);
            notifyRole(4, `Nueva inspección de o-ring requiere supervisión.`);
            res.status(201).json({ success: true, data });
        } catch (error) {
            next(error);
        }
    },
    update: async (req, res, next) => {
        try {
            const data = await InspectionOringService.update(req.params.id, req.body);
            res.json({ success: true, data });
        } catch (error) {
            next(error);
        }
    },
    remove: async (req, res, next) => {
        try {
            await InspectionOringService.delete(req.params.id);
            res.json({ success: true, message: 'Inspección de o-ring eliminada correctamente' });
        } catch (error) {
            next(error);
        }
    }
};

module.exports = InspectionOringController;