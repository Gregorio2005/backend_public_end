...
const InspectionThermoplasticsService = {
    getAll: async () => await InspectionThermoplasticsModel.findAll(),

    getById: async (id) => {
        const result = await InspectionThermoplasticsModel.findById(id);
        if (!result) throw new Error('Inspección de termoplástico no encontrada');
        return result;
    },

    create: async (data) => {
        return await InspectionThermoplasticsModel.create(data);
    },
